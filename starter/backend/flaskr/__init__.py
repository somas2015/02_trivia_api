...
def create_app(test_config=None):
  ...
  def search_question():
    ...
    try:
      selection = Question.query.filter(Question.question.ilike(look_for)).all()
  ...
  def play_quiz():
    ...
    asked_before = False
    try:
      question = Question.query.with_entities(Question.id).filter(Question.category == category_id).all()
      random_num = random.randint(0,len(question)-1)
      if question is None:
        abort(404)
      
      # print(random_num)
      # print(question[random_num][0])
      ...
      for id in ASKED_QUESTIONS:     
        if id == current_question: 
            asked_before = True
            previous_question_name = (Question.query.with_entities(Question.question).filter(Question.id == previous_question_id).one_or_none())
            break
      ...
      if asked_before == False:
        ...
        question_name =  (Question.query.with_entities(Question.question).filter(Question.id == current_question).one_or_none())
        previous_question_name = (Question.query.with_entities(Question.question).filter(Question.id == previous_question_id).one_or_none()) 
